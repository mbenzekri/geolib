...
describe('Test geojson.ts', () => {
    ...
    test('Should create Geojson', () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([data_1.paris]));
    ...
    test('Should fail to parse dummy json', () => {
        ...
        expect(async () => { for await (const f of geojson.parse()) {
            f;
        } }).rejects.toMatch('Unexpected char');
    });
    test('should parse simple geojson', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([data_1.simple]));
        let count = 0;
    ...
    test('should parse geojson with escape char', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([data_1.withescape]));
        let count = 0;
        for await (const f of geojson.parse()) {
            if (f)
                count++;
    ...
    test('should fail to parse geojson with invalid escape char', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([data_1.withescapeerr]));
        let count = 0;
        expect(async () => { for await (const f of geojson.parse()) {
            f && count++;
        } }).rejects.toMatch('syntax error valid escaped sequences');
