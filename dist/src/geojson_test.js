...
const geojsondata_1 = require("../data/geojsondata");
describe('Test geojson.ts', () => {
    beforeEach(() => null);
    test('Should create Geojson', () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([geojsondata_1.paris]));
        expect(geojson).not.toBeNull();
        expect(geojson.name).toBe('myclass');
    });
    test('Should fail to create Geojson', () => {
        expect(() => { new geojson_1.Geojson('myclass', null); }).toThrow(Error);
    });
    test('Should fail to parse dummy json', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob(['{ beurk! }']));
        let err;
        try {
    ...
    test('should parse simple geojson', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([geojsondata_1.simple]));
        let count = 0;
        for await (const f of geojson.parse()) {
            if (f)
                count++;
    ...
    test('should parse with null attribute geojson', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([geojsondata_1.withnull]));
        const all = [];
        for await (const f of geojson.parse()) {
            if (f)
                all.push(f);
    ...
    test('should parse geojson with escape char', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([geojsondata_1.withescape]));
        let count = 0;
        for await (const f of geojson.parse()) {
            if (f)
                count++;
    ...
    test('should fail to parse geojson with invalid escape char', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([geojsondata_1.withescapeerr]));
        let count = 0;
        let err;
        try {
            for await (const f of geojson.parse()) {
    ...
    test('should attribute search ordered Geojson.search()', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([geojsondata_1.simple]));
        await geojson.buildIndexes([{ attribute: 'name', type: geoindex_1.GeofileIndexType.ordered }]);
    ...
    test('should attribute fuzzy search exact Geojson.fuzzy()', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([geojsondata_1.simple]));
        await geojson.buildIndexes([{ attribute: 'name', type: geoindex_1.GeofileIndexType.fuzzy }]);
    ...
    test('should attribute prefix search Geojson.prefix()', async () => {
        const geojson = new geojson_1.Geojson('myclass', new Blob([geojsondata_1.simple]));
        await geojson.buildIndexes([{ attribute: 'name', type: geoindex_1.GeofileIndexType.prefix }]);
