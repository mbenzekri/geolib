...
const DIACRITICS = defaultDiacriticsRemovalMap.reduce(function(prev, cur) {
    for (let i = 0; i < cur.letters.length; i++) { prev[cur.letters.charAt(i)] = cur.base; }
    return prev;
}, {});
...
const CLEANFCT = [
    ...
    { name : 'Suppr liaisons     ' , op : function(value) { return !value ? value : value.replace(SUPPRRE, '', 'gi'); } },
    // tslint:disable-next-line:max-line-length
...
if (Blob) Blob.prototype.read = function(offset=0, length = this.size):Promise<ArrayBuffer> {
...
if (Blob) Blob.prototype.readDv = function(offset=0, length = this.size):Promise<DataView> {
    if (!this) return Promise.reject("null blob provided to read")
    const blob = this.slice(offset,offset + length)
    return new Promise((resolve, reject) => {
        const r = new FileReader();
...
if (Blob) Blob.prototype.readText = function(offset=0, length = this.size):Promise<string> {
    if (!this) return Promise.reject("null blob provided to read")
    const blob = this.slice(offset,offset + length)
    return new Promise((resolve, reject) => {
        const r = new FileReader();
...
String.prototype.titlecase = function (this: string): string {
    ...
    .replace(/[-\s][a-z][^-\s]*/ig,
        txt => txt.charAt(0) + txt.charAt(1).toUpperCase() + txt.substr(2)
    ).trim();
};
...
String.prototype.wordlist = function (this: string): string[] {
    if (!this) { return []; }
...
String.prototype.prefix =  function (this: string): string[] {
    if (!this) { return []; }
...
String.prototype.fuzzyhash =  function (this: string): number {
    if (!this) { return 0; }
...
String.fuzzyExtend =  function (fuzzyh: number): number[] {
 ...
 */
Array.prototype.flatten = function<T> (this: Array<T>, flat = new Array<T>()): Array<T> {
    this.forEach((v) => Array.isArray(v) ? (v as Array<T>).flatten(flat) : (v !== null && v !== undefined) ? flat.push(v) : null );
    return flat;
};

/**
 * function to fullfill an array of promise and flatten the result
 ...
 */
// Object.prototype.applyTo = function (this: Object, to: Object = {}): Object {
...
// };
// -----------------------------------------------------------------------------
// ATTENTION ! la syntaxe précédente peux générer l'erreur suivante
