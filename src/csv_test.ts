...
describe('Test csv.ts', () => {
    ...
    test('Should fail to create', () => {
        expect(() => { new Csv('myclass', null) }).toThrow(Error);
    })

    test('should parse simple', async () => {
        const csv = new Csv('myclass', new Blob([simple]))
        let count = 0
        for await (const f of csv.parse()) { if (f) count++ }
        expect(count).toEqual(3);
    })

    test('should parse simple with comment', async () => {
        const csv = new Csv('myclass', new Blob([withcomment]), {comment: '#'})
        let count = 0
        for await (const f of csv.parse()) { if (f) count++ }
        expect(count).toEqual(3);
    })
    ...
    test('should parse simple Csv with skip lines', async () => {
        const csv = new Csv('myclass', new Blob([simpleskip3]), { skip: 3 })
        let count = 0
        for await (const f of csv.parse()) { if (f) count++ }
        expect(count).toEqual(3);
    })
    ...
    test('should parse with null attribute', async () => {
        const csv = new Csv('myclass', new Blob([withnull]))
        const all = []
    ...
    test('should parse simple with header', async () => {
        const csv = new Csv('myclass', new Blob([withheader]), { header: true })
        const all = []
        for await (const f of csv.parse()) { if (f) all.push(f) }
    ...
    test('should parse simple with header and skip lines', async () => {
        const csv = new Csv('myclass', new Blob([withheaderskip3]), { header: true, skip: 3 })
        const all = []
        for await (const f of csv.parse()) { if (f) all.push(f) }
        expect(all.length).toEqual(3);
        expect(all[0].properties.lon).toBe('-100.000')
    ...
    test('should parse simple empty lines  ', async () => {
        const csv = new Csv('myclass', new Blob([withempty]))
        const all = []
        for await (const f of csv.parse()) { if (f) all.push(f) }
        expect(all.length).toEqual(3);
    })
    ...
    test('should parse csvban csv', async () => {
        const csv = new Csv('myclass', new Blob([csvban]))
        let count = 0
        for await (const f of csv.parse()) { if (f) count++ }
    ...
    test('should attribute search ordered csv.search()', async () => {
        const csv = new Csv('myclass', new Blob([withheader]), { header: true })
        await csv.buildIndexes([{ attribute: 'name', type: GeofileIndexType.ordered }])
    ...
    test('should attribute search ordered with null csv.search()', async () => {
        const csv = new Csv('myclass', new Blob([withnull]))
        await csv.buildIndexes([{ attribute: 'col2', type: GeofileIndexType.ordered }])
    ...
    test('should attribute fuzzy search exact csv.fuzzy()', async () => {
        const csv = new Csv('myclass', new Blob([withheader]), { header: true })
        await csv.buildIndexes([{ attribute: 'name', type: GeofileIndexType.fuzzy }])
    ...
    test('should attribute prefix search csv.prefix()', async () => {
        const csv = new Csv('myclass', new Blob([withheader]), { header: true })
        await csv.buildIndexes([{ attribute: 'name', type: GeofileIndexType.prefix }])
